...
const App = () => {
  const [appState, setAppState] = useState<AppState>({
    user: {
      isAuthorized: ApiApp.oidcClient.isAuthorized,
      isLoading: false
    }
  });
  ...
  useEffect(() => {
    ...
    ApiApp.oidcClient.onUserStateChanged.addListener(_onUserStateChanged);
    return () => {
      // unsubscribe from user state changes
      ApiApp.oidcClient.onUserStateChanged.removeListener(_onUserStateChanged);
    }
  }, []);
  ...
  const _onStartSignin = async () => {
    ...
    ApiApp.oidcClient.signIn(new FrontendRequestContext());  // eslint-disable-line @typescript-eslint/no-floating-promises
  }
...
function OpenIModelButton(props: OpenIModelButtonProps) {
  ...
  const _onClickSignOut = async () => {
    if (ApiApp.oidcClient)
    ApiApp.oidcClient.signOut(new FrontendRequestContext());  // eslint-disable-line @typescript-eslint/no-floating-promises
  }
